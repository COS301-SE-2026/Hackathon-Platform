...
class AnnouncementServiceTest {
    ...
    void getAnnouncements_noChannel_returnsEmpty() {
        when(channelRepo.findByChannelTypeAndEventId("EVENT_ANNOUNCEMENT", eventId)).thenReturn(Optional.empty());
    ...
    void getAnnouncements_existingChannel_returnsMessages() {
        ...
        when(channelRepo.findByChannelTypeAndEventId("EVENT_ANNOUNCEMENT", eventId)).thenReturn(Optional.of(channel));
        when(messageRepo.findTop100ByChannelIdOrderByCreatedAtDesc(channelId)).thenReturn(List.of(message));
