...
class ForumControllerTest {
  ...
  void setUp() {
    ...
    partAuth =
        new UsernamePasswordAuthenticationToken(
            part, null, List.of(new SimpleGrantedAuthority("ROLE_PARTICIPANT")));
    adminAuth =
        new UsernamePasswordAuthenticationToken(
  ...
  void getPosts_returns200() throws Exception {
    ForumAuthorResponse author =
        new ForumAuthorResponse(part.getUserId(), "Test", "Part", "PARTICIPANT");
    ...
    mockMvc
        .perform(get("/api/events/{id}/forum/posts", eventId).with(authentication(partAuth)))
  ...
  void getPost_returns200() throws Exception {
    ForumAuthorResponse author =
        new ForumAuthorResponse(part.getUserId(), "Test", "Part", "PARTICIPANT");
    ForumPostDetailResponse post =
        new ForumPostDetailResponse(postId, "TEST", "BODY", author, null, List.of());

    when(forumService.getPost(eventId, postId, part)).thenReturn(post);

    mockMvc
        .perform(
            get("/api/events/{eId}/forum/posts/{pId}", eventId, postId)
                .with(authentication(partAuth)))
        .andExpect(status().isOk());
  ...
  void createPost_Returns201() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/events/{id}/forum/posts", eventId)
                .with(authentication(partAuth))
  ...
  void createInvalidPost_returns400() throws Exception {
    ...
    verify(forumService, never())
        .createPost(eq(eventId), eq(part), any(CreateForumPostRequest.class));
  }
  ...
  void createComment_returns201() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/events/{id}/forum/posts/{postId}/comments", eventId, postId)
                .with(authentication(partAuth))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
        .andExpect(status().isCreated());
    ...
    verify(forumService)
        .createComment(eq(eventId), eq(postId), eq(part), any(CreateForumCommentRequest.class));
  }
  ...
  void getPermissions_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/{id}/forum/permissions", eventId).with(authentication(partAuth)))
        .andExpect(status().isOk());
  ...
  void deletePost_returns204() throws Exception {
    mockMvc
        .perform(
            delete("/api/events/{id}/forum/posts/{postId}", eventId, postId)
                .with(authentication(partAuth)))
  ...
  void deleteComment_returns204() throws Exception {
    mockMvc
        .perform(
            delete("/api/events/{id}/forum/comments/{commentId}", eventId, commentId)
                .with(authentication(partAuth)))
        .andExpect(status().isNoContent());
  ...
  void deletePost_admin_return204() throws Exception {
    mockMvc
        .perform(
            delete("/api/admin/events/{eid}/forum/posts/{pid}", eventId, postId)
  ...
  void deleteComment_admin_returns204() throws Exception {
    mockMvc
        .perform(
            delete("/api/admin/events/{eventId}/forum/comments/{comId}", eventId, commentId)
                .with(authentication(adminAuth)))
        .andExpect(status().isNoContent());
