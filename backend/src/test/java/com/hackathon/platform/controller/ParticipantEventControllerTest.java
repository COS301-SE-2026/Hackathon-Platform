...
class ParticipantEventControllerTest {
  ...
  void setUp() {
    ...
    user =
        User.builder()
            ...
            .firstName("Varun")
            .lastName("Dhawan")
            .email("varunDhawan@gmail.com")
            .passwordHash("helloIfYoureReadingThis")
            .status("ACTIVE")
            .role(Role.builder().roleId(2).name("PARTICIPANT").build())
            .build();
    auth =
        new UsernamePasswordAuthenticationToken(
            user, null, List.of(new SimpleGrantedAuthority("ROLE_PARTICIPANT")));
  }
  ...
  void getOpenEvents_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/open").with(authentication(auth)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
        .andExpect(jsonPath("$[0].eventId").value(eventId.toString()))
        .andExpect(jsonPath("$[0].name").value("Open Event"));
  ...
  void getUserActiveEvents_return200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/user-active-events").with(authentication(auth)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].name").value("Active Event"));
  }
  ...
  void getEventById_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/{eventId}", eventId).with(authentication(auth)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.eventId").value(eventId.toString()))
        .andExpect(jsonPath("$.name").value("FLY AT UP"));
  }
  ...
  void registerForEvent_withReq_returns201() throws Exception {
    ...
    EventRegistrationResponse resp =
        new EventRegistrationResponse(
            UUID.randomUUID(), eventId, Instant.now(), "Vegetarian", "Peanuts");
    when(eventRegService.registerForEvent(eventId, userId, "REG123", "Vegetarian", "Peanuts"))
        .thenReturn(resp);
    mockMvc
        .perform(
            post("/api/events/{eventId}/registered", eventId)
                .with(authentication(auth))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objMapper.writeValueAsString(evReq)))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.eventId").value(eventId.toString()))
        .andExpect(jsonPath("$.dietaryReq").value("Vegetarian"))
  ...
  void registerForEvent_withoutReq() throws Exception {
    EventRegistrationResponse resp =
        new EventRegistrationResponse(UUID.randomUUID(), eventId, Instant.now(), null, null);
  ...
  void getMyRegistrations_returns200() throws Exception {
    EventRegistrationResponse resp =
        new EventRegistrationResponse(UUID.randomUUID(), eventId, Instant.now(), "None", "None");
    when(eventRegService.getMyRegistrations(userId)).thenReturn(List.of(resp));
    mockMvc
        .perform(get("/api/events/my-registrations").with(authentication(auth)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
        .andExpect(jsonPath("$[0].eventId").value(eventId.toString()));
