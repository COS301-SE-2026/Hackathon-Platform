...
class AnnouncementControllerTest {
  ...
  void getAnnouncements_participants_returns200() throws Exception {
    mockMvc
        .perform(get("/api/events/{id}/announcements", eventId).with(authentication(partAuth)))
  ...
  void getAnnouncements_admins_returns200() throws Exception {
    mockMvc
        .perform(
            get("/api/admin/events/{id}/announcements", eventId).with(authentication(adminAuth)))
        .andExpect(status().isOk());
  ...
  void createAnnouncement_admin_returns201() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/admin/events/{id}/announcements", eventId)
    ...
    verify(annSer)
        .createAnnouncement(
            eq(eventId), eq(admin.getUserId()), any(CreateAnnouncementRequest.class));
  }
  ...
  void createAnnouncement_participant_returns403() throws Exception {
    ...
    verify(annSer, never())
        .createAnnouncement(
            eq(eventId), eq(part.getUserId()), any(CreateAnnouncementRequest.class));
  }
}
