...
class LevelControllerTest {
    ...
    void setUp(){
        ...
        User ad = User.builder().userId(UUID.randomUUID()).firstName("ShahRukh").lastName("Khan").email("srk@gmail.com").passwordHash("$2a$12$hashedpassword").role(adR).status("ACTIVE").build();
        User svdAdmin = usrRep.saveAndFlush(ad);

        User partici = User.builder().userId(UUID.randomUUID()).firstName("Virat").lastName("Kohli").email("vk@gmail.com").passwordHash("$2a$12$hashedpassword").role(pR).status("ACTIVE").build();
        User svdP = usrRep.saveAndFlush(partici);

        admin = new UsernamePasswordAuthenticationToken(svdAdmin, null, List.of(new SimpleGrantedAuthority("ROLE_ADMIN")));
        part = new UsernamePasswordAuthenticationToken(svdP, null, List.of(new SimpleGrantedAuthority("ROLE_PARTICIPANT")));
