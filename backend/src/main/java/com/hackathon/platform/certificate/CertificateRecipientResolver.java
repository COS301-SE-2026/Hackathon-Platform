...
public class CertificateRecipientResolver{
    ...
    public CertificateRecipientResolver(TeamRepository teamRepo, TeamMemberRepository teamMemRepo, UserRepository userRepo, LeaderboardService leaderboard) {
    ...
    public List<CertificateRecipient> resolve(Event event, String scope, Integer topN){
        List<LeaderboardEntryResponse> leaderboard = leaderboard.getEventLeaderBoard(event.getEventId());
        ...
        for(Team team : teams){
            ...
            for(TeamMember member : members){
                ...
                recipients.add(new CertificateRecipient(user.getUserId(), team.getTeamId(), fullName, rank, certificateType, fields));
            }
        }
        return recipients;
    }
