...
public class WorkspaceTelemetryService {
  ...
  public UUID startSession(UUID workspaceId, User user) {
    ...
    List<IdeTelemetrySession> openSessions =
        sessionRepo.findByWorkspaceIdAndUserIdAndEndedAtIsNull(workspaceId, user.getUserId());
    ...
    IdeTelemetrySession session =
        IdeTelemetrySession.builder()
  ...
  public void saveBatch(UUID workspaceId, TelemetryBatchRequest req, User user) {
    ...
    for (TelemetryEventRequest reqEvent : reqs) {
      ...
      IdeTelemetryEvent event =
          IdeTelemetryEvent.builder()
              .eventId(UUID.randomUUID())
              .sessionId(session.getSessionId())
              .workspaceId(workspaceId)
              .userId(user.getUserId())
              .eventType(reqEvent.eventType().trim())
              .clientTimestamp(reqEvent.clientTimestamp())
              .receivedAt(now)
              .sequenceNumber(reqEvent.sequenceNumber())
              .payload(safePayload(reqEvent.payload()))
              .build();
      events.add(event);
    }
  ...
  private IdeTelemetrySession requireValidSession(UUID workspaceId, UUID sessionId, User user) {
    ...
    IdeTelemetrySession session =
        sessionRepo
            .findById(sessionId)
            .orElseThrow(() -> new IllegalArgumentException("Session could not be found"));

    if (!session.getWorkspaceId().equals(workspaceId)
        || !session.getUserId().equals(user.getUserId())) {
