...
public class SubmissionCreationService {
    ...
    public Map<String, String> createSubmission(String eventId, String teamId, MultipartFile outputFile, MultipartFile sourceFile, short levelId, User currUser) {
        ...
        boolean teamMember = teamMemberRepo.findByUserIdAndStatusAndEventId(currUser.getUserId(), "APPROVED", eventUUID).stream().anyMatch(member -> teamUUID.equals(member.getTeamId()));

        if (!teamMember) {
            throw new StorageException("You are not part of this team");
        }

        Level level = levelRepo.findById(levelId).orElseThrow(() -> new StorageException("Level not found"));

        if (!hackathonIdMatchesEvent(level.getHackathonId(), event.getHackathon())) {
            throw new StorageException("Level doesnt exist");
        }

        UUID hackathonId = eventRepo.findHackathonIdByEventId(eventUUID).orElseThrow(() -> new StorageException("Hackathon could not be found for event: " + eventId));

        SolverVersion latestSolver = solverVersionRepository.findByHackathonIdAndIsActiveTrue(hackathonId).orElseThrow(() -> new StorageException("No active solver has been uploaded yet"));

        Submission saved = metaService.saveSubmission(eventId, teamUUID, levelId, latestSolver.getId(), outputFile.getOriginalFilename(), outputFile.getSize(), outputFile.getContentType(), sourceFile.getOriginalFilename(), sourceFile.getSize(), sourceFile.getContentType());

        storeService.upload(config.getSubmissionsContainer(), saved.getOutputStorageKey(), outputFile);
        storeService.upload(config.getSubmissionsContainer(), saved.getSourceCodeStorageKey(), sourceFile);

        String record = producer.enqueue(saved.getId());

        return Map.of("submissionId", String.valueOf(saved.getId()), "outputStorageKey", saved.getOutputStorageKey(), "sourceStorageKey", saved.getSourceCodeStorageKey(), "status", "QUEUED", "scoringRecordId", record != null ? record : "");
    }
