...
public interface StorageService {
   ...
   * Always use presigned URLs when serving files to clients — never expose raw blob URLs.
   *
   * @param containerName the Azure blob container name
  ...
  /**
   * Deletes a blob from storage (e.g. when an admin replaces a solver or level file).
   *
   * @param containerName the Azure blob container name
   * @param storageKey    full storage key path
   */
