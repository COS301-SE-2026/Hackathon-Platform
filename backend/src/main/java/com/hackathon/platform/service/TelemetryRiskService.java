...
public class TelemetryRiskService {
  ...
  public TelemetryRiskReport analyze(UUID workspaceId, UUID userId) {
    TelemetryFeatureService.TelemetryFeatures features =
        featureService.extractFeatures(workspaceId, userId);
    ...
    if (features.largePasteAfterTabReturnCount() > 0) {
      indicators.add(
          features.largePasteAfterTabReturnCount()
              + " large paste event(s) occurred shortly after returning to IDE");
    }

    if (features.pastedCharacters() >= 100 && features.reworkRatio() < 0.1) {
      double percentage = features.reworkRatio() * 100;
      indicators.add(
          String.format("Low rework observed relative to inserted code: %.1f%%", percentage));
    }

    return indicators;
  }

  public record TelemetryRiskReport(
      ...
      TelemetryFeatureService.TelemetryFeatures features,
      AiModelClient.AiPredictionResponse aiPrediction) {}
}
