...
public class CodeWorkspaceService {
  ...
  public CodeWorkspace getOrCreateWorkspace(UUID eventId, UUID teamId, short levelId, User user) {
    ...
    return workspaceRepo
        .findByEventIdAndTeamIdAndLevelId(eventId, teamId, levelId)
        .orElseThrow(() -> new IllegalStateException("Workspace could not load"));
  }
  ...
  public CodeWorkspace getWorkspaceForUser(UUID workspaceId, User user) {
    ...
    CodeWorkspace work =
        workspaceRepo
            ...
            .orElseThrow(() -> new IllegalArgumentException("Could not find the workspace"));

    UUID hackId =
        accService.requireParticipantAccess(
            work.getEventId(), work.getTeamId(), work.getLevelId(), user.getUserId());
