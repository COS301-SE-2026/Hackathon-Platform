...
public class AnnouncementService {
    ...
    public CreateAnnouncementResponse createAnnouncement(UUID eventId, UUID adminId, CreateAnnouncementRequest req) {
        ...
        List<AnnouncementEmailDelivery> deliveries = users.stream().map(user -> createEmailDelivery(savedMessage.getMessageId(), user)).toList();
    ...
    private CommunicationChannel getOrCreateEventAnnouncementChannel(UUID eventId) {
        return channelRepo.findByChannelTypeAndEventId(EVENT_ANNOUNCEMENT, eventId).orElseGet(() -> {
    ...
    private List<AnnouncementResponse> getAnnouncementsForEvent(UUID eventId) {
        return channelRepo.findByChannelTypeAndEventId(EVENT_ANNOUNCEMENT, eventId).map(channel -> messageRepo.findTop100ByChannelIdOrderByCreatedAtDesc(channel.getChannelId()).stream().map(message -> toResponse(message, eventId)).toList()).orElseGet(Collections::emptyList);
    }
