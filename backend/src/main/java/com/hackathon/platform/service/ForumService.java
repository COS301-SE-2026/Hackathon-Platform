...
public class ForumService {
  ...
  public List<ForumPostSummaryResponse> getPosts(UUID eventId, User user) {
    ...
    return postRepo.findByEventIdAndIsDeletedFalseOrderByCreatedAtDesc(eventId).stream()
  ...
  public ForumPostDetailResponse getPost(UUID eventId, UUID postId, User user) {
    ...
    List<ForumCommentResponse> comments =
        commentRepo.findByPostIdAndIsDeletedFalseOrderByCreatedAtAsc(postId).stream()
  ...
  public ForumCommentResponse createComment(
      UUID eventId, UUID postId, User user, CreateForumCommentRequest req) {
    accessService.requireForumAccess(eventId, user);
    ForumPost post = requirePost(eventId, postId);
    ...
    eventPublisher.publishEvent(
        new ForumUpdatedEvent(eventId, "COMMENT_CREATED", saved.getCommentId()));
  ...
  public void deleteComment(UUID eventId, UUID commentId, User user) {
    ...
    eventPublisher.publishEvent(
        new ForumUpdatedEvent(eventId, "COMMENT_DELETED", comment.getCommentId()));
  }

  private ForumPost requirePost(UUID eventId, UUID postId) {
    return postRepo
        .findByPostIdAndEventIdAndIsDeletedFalse(postId, eventId)
        .orElseThrow(() -> new IllegalArgumentException("Post: " + postId + " could not be found"));
  }
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Forum author: " + userId + " could not be found"));
