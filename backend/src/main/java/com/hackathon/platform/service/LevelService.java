...
public class LevelService {
  ...
  public Level createLevel(UUID hackathonId, LevelRequest req) {
    ...
    if (levelRepository.existsByHackathonIdAndLevelNumber(hackathonId, req.getLevelNumber())) {
      throw new IllegalArgumentException(
          "level number " + req.getLevelNumber() + " already exists");
    }
  ...
  public Level updateLevel(short id, LevelRequest req) {
    ...
    if (req.getLevelNumber() != null
        && req.getLevelNumber() > 0
        && req.getLevelNumber() != level.getLevelNumber()) {
      if (levelRepository.existsByHackathonIdAndLevelNumber(
          level.getHackathonId(), req.getLevelNumber())) {
        throw new IllegalArgumentException(
            "level number " + req.getLevelNumber() + " already exists");
      }
