...
public class TelemetryFeatureService {
  ...
  public TelemetryFeatures extractFeatures(UUID workspaceId, UUID userId) {
    ...
    for (IdeTelemetryEvent event : events) {
      eventsBySession
          .computeIfAbsent(event.getSessionId(), ignored -> new ArrayList<>())
          .add(event);
    }
    ...
    if (accumulator.typingIntervalWeight > 0) {
      avgTypingInterval =
          accumulator.typingIntervalWeightedTotal / accumulator.typingIntervalWeight;
    }
  ...
  private void processSession(List<IdeTelemetryEvent> events, FeatureAccumulator accumulator) {
    ...
    for (IdeTelemetryEvent event : events) {
      ...
      switch (eventType) {
        ...
        case "PASTE" -> {
          ...
          accumulator.pasteEvents++;
          accumulator.largestPasteCharacters =
              Math.max(accumulator.largestPasteCharacters, characters);
          ...
          if (timestamp != null) {
            ...
            if (tabHiddenAt != null) {
              ...
              if (awaySeconds >= 0) {
                ...
                accumulator.maxTabAwaySeconds =
                    Math.max(accumulator.maxTabAwaySeconds, awaySeconds);
              }
              tabHiddenAt = null;
            }
          }
    ...
    if (firstTimestamp != null
      ...
      accumulator.activeDurationSeconds +=
          Duration.between(firstTimestamp, lastTimestamp).getSeconds();
    }
  }
