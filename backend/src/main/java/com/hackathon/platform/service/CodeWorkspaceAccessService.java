...
public class CodeWorkspaceAccessService {
    ...
    public UUID requireParticipantAccess(UUID eventId, UUID teamId, short levelId, UUID userId) {
        ...
        boolean member = teamMemRepo.findByUserIdAndStatusAndEventId(userId, "APPROVED", eventId).stream().anyMatch(m -> teamId.equals(m.getTeamId()));

        if(!member) {
            throw new AccessDeniedException("You are not appart of this team");
        }

        Level lvl = levelRepo.findById(levelId).orElseThrow(() -> new IllegalArgumentException("Level could not be found"));

        UUID hackId = eventRepo.findHackathonIdByEventId(eventId).orElseThrow(() -> new IllegalArgumentException("Hackathon could not be found"));
