...
public class WorkspaceSubmissionService {
    ...
    public Map<String, String> createSubmission(UUID workspaceId, User user) {
        ...
        MultipartFile outputFile = new ByteArrayMultipartFile("outputFile", "output.json", "application/json", submissionPackage.outputJson());
        MultipartFile sourceFile = new ByteArrayMultipartFile("sourceFile", "source.zip", "application/zip", submissionPackage.sourceZip());
        return createService.createSubmission(workspace.getEventId().toString(), workspace.getTeamId().toString(), outputFile, sourceFile, workspace.getLevelId(), user);
    }
}