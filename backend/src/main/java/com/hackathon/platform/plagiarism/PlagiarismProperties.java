...
/** Config for the batch plagiarism check. for the top N teams on a leaderboard. */
@Component
...
public class PlagiarismProperties {
  ...
  /**
   * When true, a pair is also flagged is its combined score is a stat outlier relative to every
   * other compared pair in the same level.
   */
  ...
  /**
   * Minimun (corpus-centered) cosine similarity for a function pair to be surfaced in the diff
   * view's function match list
   */
