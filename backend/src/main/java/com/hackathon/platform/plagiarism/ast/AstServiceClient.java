...
public class AstServiceClient {
    ...
    /** Calls /parse. Returns a response with status "service_unavailable" (an
     * internal-only status, never sent by the Python side) on any network/timeout/deserialization
     * failure or non-2xx status code. Callers should treat this identically to
     * "unsupported_language" / "parse_error", i.e. fall back to the regex lexer.
     * 
    */
    public AstParseResponse parse(String fileName, String content) {
        
        if(!props.isEnabled()) {
            return new AstParseResponse("service_unavailable", null, List.of(), List.of(), "ast service disabled");
        ...
        try{
            ...
            logger.warn("AST service /parse call failed for {}, falling back to lexer: {}", fileName, e.getMessage());
            return new AstParseResponse("service_unavailable", null, List.of(), List.of(), e.getMessage());
    ...
    /**
     * Calls /embed for a batch of function spans belonging to one file's source. Returns
     * an empty list if embedding backend unavailable. Run doesnt fail in that case, just 
     * based on strucutrual score alone.
     */
    public List<FunctionEmbedding> embed(String fileName, String content, List<AstFunctionSpan> spans) {
        ...
        try {
            List<SpanRequestBody> spanBodies =
                ...
                if(!"ok".equals(wire.status())) {
                    logger.warn("AST service /embed reported status={} for {}: {}", wire.status(), fileName, wire.detail());
                    return List.of();
                }
            ...
            logger.warn("AST service /embed call failed for {}, skipping embeddings: {}", fileName, e.getMessage());
            return List.of();
        }
