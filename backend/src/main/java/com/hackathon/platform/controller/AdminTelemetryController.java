...
public class AdminTelemetryController {
  ...
  public TelemetryRiskService.TelemetryRiskReport getReport(
  ...
  @PreAuthorize("hasAnyRole('ADMIN', 'SUPERADMIN')")
  public List<AdminWorkspaceResponse> getTeamWorkspaces(
      @PathVariable UUID eventId, @PathVariable UUID teamId) {
    return workspaceRepo.findByEventIdAndTeamIdOrderByLevelIdAsc(eventId, teamId).stream()
        .map(
            workspace -> {
              var level =
                  levelRepo
                      .findById(workspace.getLevelId())
                      .orElseThrow(() -> new IllegalStateException("Level could not be found"));
              return new AdminWorkspaceResponse(
                  workspace.getWorkspaceId(), workspace.getLevelId(), level.getLevelNumber());
            })
        .toList();
  }
