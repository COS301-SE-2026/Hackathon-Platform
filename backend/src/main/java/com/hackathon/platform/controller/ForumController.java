...
public class ForumController {
    ...
    public ResponseEntity<ForumPostDetailResponse> getPost(@PathVariable UUID eventId, @PathVariable UUID postId, @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(forumService.getPost(eventId, postId, user));
    }

    @PostMapping("/posts")
    public ResponseEntity<ForumPostDetailResponse> createPost(@PathVariable UUID eventId, @AuthenticationPrincipal User user, @Valid @RequestBody CreateForumPostRequest req) {
        return ResponseEntity.status(HttpStatus.CREATED).body(forumService.createPost(eventId, user, req));
    }

    @PostMapping("/posts/{postId}/comments")
    public ResponseEntity<ForumCommentResponse> createComment(@PathVariable UUID eventId, @PathVariable UUID postId, @AuthenticationPrincipal User user, @Valid @RequestBody CreateForumCommentRequest req) {
        return ResponseEntity.status(HttpStatus.CREATED).body(forumService.createComment(eventId, postId, user, req));
    }
